...
func echoHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.TLS != nil {
		...
		// mTLS: show client certificate details if present
		if len(r.TLS.PeerCertificates) > 0 {
			...
			tlsInfo["client_cert"] = nil // no client cert presented
		}
	}
...
func rootHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "TLS Test Server is running.\nUse /health or /echo for testing.\n")
}
...
func main() {
	...
	// CLIENT_CA_FILE: path to the CA cert (or client cert itself if self-signed)
	// that signed the Choreo proxy's client certificate.
	// If set, mTLS is enabled — the server will require and verify client certs.
	// If not set, regular one-way TLS is used.
	...
	if clientCAFile != "" {
		...
		if !pool.AppendCertsFromPEM(caCert) {
			log.Fatalf("Failed to parse client CA cert from %s", clientCAFile)
		}
		cfg.ClientCAs = pool
		...
		log.Printf("mTLS disabled — one-way TLS only (set CLIENT_CA_FILE to enable mTLS)")
	}
